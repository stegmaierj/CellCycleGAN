...
settings.inputFolder = [settings.sourceDir '../Data/']; %% path to the *.h5 files extracted from the SI of Zhong et al., 2012, NMeth
settings.outputFolder = [settings.sourceDir '../Processing/']; %% output folder for temporary and generated image data
settings.shapeModelPath = [settings.sourceDir 'shapeModel.mat']; %% shape model path
settings.pretrainedModelPath = [settings.sourceDir 'pretrainedModel.ckpt'];
...
settings.outputFolderLabel = sprintf('%s%02d_GT/TRA/', settings.outputFolder, settings.randomSeed);
settings.outputFolderRaw = sprintf('%s%02d/', settings.outputFolder, settings.randomSeed);
